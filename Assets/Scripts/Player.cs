...
public class Player : Actor, IExperience
{
	...
	private GameObject equiped = null;
	...
	void Update()
	{
		if (equiped != null) 
		{
			...
			equiped.transform.forward = this.transform.forward;
			equiped.transform.position = this.transform.position + offset;
			equiped.transform.LookAt (this.transform.position);
		}
	}
	...
	{
		return myInventory.findSpecificItem (item.name);
	}
